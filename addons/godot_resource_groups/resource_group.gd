...
var loaded_paths:Dictionary[String, Resource] = {}
...
func get_matching_resource(includes:Array[String], excludes:Array[String] = []) -> Array[Resource]:
	if loaded_paths.size() == 0:
		load_paths()
	var path_verifier = PathVerifier.new(base_folder, includes, excludes)
	var filtered_paths = paths.filter(func(it): return path_verifier.matches(it))
	var matches: Array[Resource] = []
	for path in filtered_paths:
		matches.append(loaded_paths[path])
	return(matches)
